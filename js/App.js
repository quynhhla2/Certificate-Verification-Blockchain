...
window.CONTRACT = {
  address: "0xedd6911c876592357b87916c8ebbf04bd72acd60",
...
async function get_Sha3() {
  $("#note").html(`<h5 class="text-warning">Đang mã hóa 😴...</h5>`);
  ...
  if (file) {
    ...
    reader.onload = async function (evt) {
      ...
      $("#note").html(
        `<h5 class="text-center text-info">Chứng chỉ đã được mã hóa  😎 </h5>`
      );
    };
...
function printUploadInfo(result) {
  ...
  $("#note").html(`<h5 class="text-info">
   Giao dịch thành công 😊<i class="mx-2 text-info fa fa-check-circle" aria-hidden="true"></i>
   </h5>`);
  listen();
}
...
async function uploadFileToIpfs() {
  ...
  //for authinticating your request to infura
...
async function sendHash() {
  ...
  $("#note").html(`<h5 class="text-info">Hãy xác nhận giao dịch 🙂</h5>`);
  $("#upload_file_button").attr("disabled", true);
  get_ChainID();
  // Initilize Ipfs
  ...
  if (window.hashedfile.length > 4) {
    await window.contract.methods
      ...
      .on("transactionHash", function (_hash) {
        $("#note").html(
          `<h5 class="text-info p-1 text-center">Đợi hoàn tất giao dịch...</h5>`
        );
      })
  ...
  if (window.hashedfile) {
    await window.contract.methods
      ...
      .on("transactionHash", function (hash) {
        $("#note").html(
          `<h5 class="text-info p-1 text-center">Đang hoàn tất 😴...</h5>`
        );
      })
...
async function get_ChainID() {
  ...
  switch (a) {
    ...
    case 42:
      ...
      break;
    default:
      window.chainID = "ChainID không xác định";
      break;
  }
...
async function addExporter() {
  ...
  if (info && address) {
    ...
    try {
      await window.contract.methods
        ...
        .on("receipt", function (receipt) {
          ...
          console.log(receipt);
          $("#note").html(
            `<h5 class="text-info">Tổ chức đã được thêm vào 😇</h5>`
          );
        })
    ...
    $("#note").html(
      `<h5 class="text-center text-warning">Thêm thông tin và địa chỉ của tổ chức  </h5>`
    );
  }
}
...
async function editExporter() {
  ...
  if (info && address) {
    ...
    $("#note").html(
      `<h5 class="text-center text-warning">Thêm thông tin và địa chỉ của tổ chức cần xóa 😵‍💫 </h5>`
    );
  }
}
...
async function deleteExporter() {
  ...
  if (address) {
    ...
    $("#note").html(
      `<h5 class="text-center text-warning">Thêm địa chỉ của tổ chức cần xóa 👍</h5>`
    );
  }
}
